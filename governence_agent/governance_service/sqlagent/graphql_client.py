"""
Production miniERP GraphQL client.
...
Exposes the two read operations documented in DB_API_documentation_v1.1.md:
  - find_with_offset_pagination(table, options) -> dict
  - find_with_cursor_pagination(table, options) -> dict

Both return the full GraphQL response payload for the operation
...
async def get_sales_order_data_by_order_number(
  ...
  getSalesOrderDataByOrderNumber({field}: {json.dumps(value)}) {{
    ...
    data = await _post_graphql(query, {}, operation_name=None)
