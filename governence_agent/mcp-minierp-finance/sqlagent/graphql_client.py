"""
Production miniERP GraphQL client (finance backend).

Same client as mcp-minierp/sqlagent/graphql_client.py, minus the
order-specific custom-field helper (get_sales_order_data_by_order_number) --
finance has no equivalent custom field and uses the two generic pagination
operations only.
...
# Module-level cache. asyncio.Lock guards concurrent refresh attempts.
...
async def find_with_offset_pagination(
    ...
    Args:
      table:   Entity name (e.g. "Account", "APInvoice", "Vendor").
